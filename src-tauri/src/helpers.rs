...
pub fn decode_buffer(buf: Vec<u8>) -> (String, String, String) {
    ...
    let potential_encodings = [
        first_encoding.as_str(),
        second_encoding.as_str(),
        "UTF-8",
        "Windows-1251",
    ];

    let mut best_result = ("Decoding failed".to_string(), "".to_string(), "".to_string());
    let mut lowest_unprintable_count = usize::MAX;
    ...
    for &encoding_label in &potential_encodings {
        if let Some(encoding) = encoding_from_whatwg_label(encoding_label) {
            if let Ok(decoded) = encoding.decode(&buf, DecoderTrap::Replace) {
                let unprintable_count = decoded.chars().filter(|c| !c.is_control() && !c.is_whitespace()).count();
                if unprintable_count < lowest_unprintable_count {
                    lowest_unprintable_count = unprintable_count;
                    best_result = (decoded, "".to_string(), "".to_string());
                }
            }
        }
    }
